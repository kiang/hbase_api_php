...
if (!file_exists($scannerFile)) {
    ...
    curl_setopt($ch, CURLOPT_URL, "http://hcdnc611:20550/news/scanner");
        ...
        'filter' => json_encode(array(
            ...
            'filters' => array(
                array(
                    ...
                    'latestVersion' => true,
                    'ifMissing' => true,
                    'qualifier' => base64_encode('body'),
                    'family' => base64_encode('main'),
                    'op' => 'EQUAL',
                    'comparator' => array(
    ...
    $parts = explode('Location: http://hcdnc611:20550/news/scanner/', $s);
    $parts = explode("\n", $parts[1]);
    ...
    while ($hasNext) {
        ...
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, "http://hcdnc611:20550/news/scanner/" . $parts[0]);
        curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
        curl_setopt($ch, CURLOPT_HTTPAUTH, CURLAUTH_GSSNEGOTIATE);
        curl_setopt($ch, CURLOPT_USERPWD, ":");
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        ...
        if (empty($json->Row)) {
            ...
            file_put_contents(__DIR__ . '/tmp/result_' . $pageCount, json_encode($json, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE));
